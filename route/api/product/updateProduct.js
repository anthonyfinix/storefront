...
module.exports = async({
    ...
    try {
        ...
        if (suppliers) {
            ...
            let dbSuppliers = await getManySupplier(stores.map((supplier) => ({ _id: supplier.id })));
            if (dbSuppliers.error) return { error };
            console.log(dbSuppliers)
            for (let supplier of suppliers) {
                let supplierIdFound = false;
                for (let supplierId of dbSuppliers.result) {
                    if (supplier.id == supplierId._id) supplierIdFound = true;
                }
                if (!supplierIdFound) return { error: `supplier id provided not found` };
            }
